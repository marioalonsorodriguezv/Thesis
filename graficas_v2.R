...
p1 <- ggplot(data=gr_1, aes(x=year, y=idea_pct, colour=un_region_name)) + geom_line()
...
p1 <- p1 + guides(color=guide_legend("Region")) + theme(legend.position="bottom", legend.background = element_rect(colour = "grey"))  
p1 
############################################################################
...
p2 <- ggplot(gr_2_bar_r,aes(un_region_name,value,fill=subindex))+
  geom_bar(stat="identity",position="dodge", colour="black") + facet_grid(~year) + theme_bw()



p2 <- p2 + labs(title = "PFR Subindex level in 1996 & 2015", x = "Region", y = "PFR Subindex") 
p2 <- p2 + scale_fill_brewer(name="PFR\nSubindex", labels=c("Bans & limits\non private income", "Public funding", "Regulation\non spending","Reporting, oversight\nand sanctions"))
p2 <- p2 + theme(legend.position="bottom", legend.background = element_rect(colour = "grey"))  
p2 <- p2 + theme(panel.border=element_rect(fill=NA)) + theme(plot.background = element_rect(colour = "black", size = 0.5))
p2 <- p2 + theme(plot.title = element_text(hjust = 0.5))
p2 <- p2 + theme(strip.background=element_rect(fill="white"))
p2
############################################################################
...
# --> (3) Gr??fico de barras diferencia PFR 1996-2015, dividido por categor??a y 
# dentro de cada categor??a las cinco regiones.
############################################################################
df <- read.csv("AnalisisRCompleta.csv")
...
p3 <- p3 + scale_fill_manual(name = "", labels=c("World Bank Control of Corruption", "PFR Index"), values = c("dodgerblue","dodgerblue4"))
p3 <- p3 + theme(legend.position="bottom", legend.background = element_rect(colour = "grey"))  
p3 <- p3 + theme(panel.border=element_rect(fill=NA)) + theme(plot.background = element_rect(colour = "black", size = 0.5))
